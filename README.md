...
- 设置电流 (默认 20A)
- 输出开关控制
- 断开连接时会先切回本地控制，再断开远程连接
- 实时电压/电流显示（由后端后台采集并缓存）
...
- 平均值显示 (mV/mA)
- 自动刷新（后端后台保活并采集，前端只读取缓存）
- 所有通道关闭时停止工作页数值轮询
- 连接 / 自动重连 / 健康检查会输出 `[scope]` 日志，便于排查 session 失效问题
...
- 设备连接状态和采集结果由后端统一维护在内存状态中，页面切换或刷新不会主动重新连接设备。
- 设备管理页仍会自动刷新串口和电源资源列表，但电源 VISA 资源扫描已改为子进程隔离执行，避免干扰已打开的示波器会话。
